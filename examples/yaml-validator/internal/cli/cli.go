...
import (
	...
	"github.com/NickMoignard/yamlvalidate/internal/version"
)
...
func NewRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     binaryName,
		Short:   binaryName + " — validate YAML files against a JSON Schema",
		Version: version.Short(),
		// The root itself does no work; reject stray positional args so an unknown
		// command/arg is a usage error (2), not silently swallowed.
		Args: cobra.NoArgs,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			// Bare invocation shows help; the real work lives in `validate`.
			return cmd.Help()
		},
	}
